...
class DexCubit extends Cubit<DexState> {
  ...
  Future<void> updateDex(String tokenFrom, String tokenTo, double? amountFrom, double? amountTo, String address, List<AddressModel> addressList, TokensState tokensState) async {
    TestPoolSwapModel dexModel;
    emit(DexLoadingState());
    if (tokenFrom == tokenTo) {
      dexModel = TestPoolSwapModel(tokenFrom: tokenFrom, tokenTo: tokenTo, amountFrom: 0, amountTo: 0, priceFrom: 1, priceTo: 1);
      emit(DexLoadedState(dexModel));
    } else {
      try {
        dexModel = await _dexHelper.calculateDex(tokenFrom, tokenTo, amountFrom, amountTo, address, addressList, tokensState);
        emit(DexLoadedState(dexModel));
      } catch (err) {
        emit(DexErrorState());
      }
