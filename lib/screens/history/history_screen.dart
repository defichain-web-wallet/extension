...
import 'package:defi_wallet/widgets/history/filter_list.dart';
                        ...
                        if (SettingsHelper.settings.network == 'testnet') {
                          if (state.historyFilterBy == 'receive') {
                            ...
                            testnetFilteredList = state
                                .activeAccount!.testnetHistoryList!
                                .where((element) =>
                                    element.type == 'AddPoolLiquidity');
                          } else if (state.historyFilterBy ==
                              'remove liquidity') {
                            testnetFilteredList = state
                                .activeAccount!.testnetHistoryList!
                                .where((element) =>
                                    element.type == 'RemovePoolLiquidity');
                          } else if (state.historyFilterBy ==
                              'account to utxos') {
                            testnetFilteredList = state
                                .activeAccount!.testnetHistoryList!
                                .where((element) =>
                                    element.type == 'AccountToUtxos');
                          } else if (state.historyFilterBy ==
                              'utxos to account') {
                            testnetFilteredList = state
                                .activeAccount!.testnetHistoryList!
                                .where((element) =>
                                    element.type == 'UtxosToAccount');
                          } else {
                            testnetFilteredList =
                          ...
                          if (state.historyFilterBy == 'receive') {
                            filteredList = state.activeAccount!.historyList!
                                .where(
                                    (element) => element.category == 'RECEIVE');
                          } else if (state.historyFilterBy == 'send') {
                            filteredList = state.activeAccount!.historyList!
                            ...
                            filteredList = state.activeAccount!.historyList!
                                .where((element) =>
                                    element.category == 'PoolSwap');
                          } else if (state.historyFilterBy == 'add liquidity') {
                            filteredList = state.activeAccount!.historyList!
                                .where((element) =>
                                    element.category == 'AddPoolLiquidity');
                          } else if (state.historyFilterBy ==
                              'remove liquidity') {
                            filteredList = state.activeAccount!.historyList!
                                .where((element) =>
                                    element.category == 'RemovePoolLiquidity');
                          } else if (state.historyFilterBy ==
                              'account to utxos') {
                            filteredList = state.activeAccount!.historyList!
                                .where((element) =>
                                    element.category == 'AccountToUtxos');
                          } else if (state.historyFilterBy ==
                              'utxos to account') {
                            filteredList = state.activeAccount!.historyList!
                                .where((element) =>
                                    element.category == 'UtxosToAccount');
                          } else {
                            filteredList = state.activeAccount!.historyList!;
                          }
                                          ...
                                          itemBuilder: (context, index) {
                                            ...
                                            if (SettingsHelper.isBitcoin()) {
                                              ...
                                              date = formatter.format(DateTime
                                                  ...
                                                  .toLocal());
                                              txValuePrefix = (type == 'SEND' ||
                                                      type == 'RECEIVE')
                                                  ? isSend
                                                      ? ''
                                                      : '+'
                                                  : '';
                                            } else if (SettingsHelper
                                                    .settings.network ==
                                                'mainnet') {
                                              tokenName = historyList[index]
                                                  .tokens![0]
                                                  .code;
                                              txValue =
                                                  historyList[index].value;
                                              ...
                                              tokenName =
                                                  testnetHistoryList[index]
                                                      .token;
                                              txValue = convertFromSatoshi(
                                                  testnetHistoryList[index]
                                                      .value);
                                              isSend = testnetHistoryList[index]
                                                  .isSend;
                                              type = testnetHistoryList[index]
                                                  .type;
                                              date = (testnetHistoryList[index]
                                                          .blockTime !=
                                                      null)
                                                  ? formatter.format(
                                                ...
                                                child: ListTile(
                                                  ...
                                                  title: Row(
                                                    ...
                                                    children: [
                                                      ...
                                                      Text(
                                                        '$txValuePrefix${balancesHelper.numberStyling(
                                                          txValue,
                                                          fixed: true,
                                                          fixedCount: 8,
                                                        )} $tokenName',
