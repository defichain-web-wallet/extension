...
class _SelectPoolState extends State<SelectPool> with ThemeMixin {
  ...
  final TextEditingController _amountBaseController = TextEditingController(text: '0');
  final TextEditingController _amountQuoteController = TextEditingController(text: '0');
        ...
        return BlocBuilder<AccountCubit, AccountState>(builder: (accountContext, accountState) {
          if (tokensState.status == TokensStatusList.success) {
            if (accountState.status == AccountStatusList.success) {
              ...
              try {
                var baseBalance = List.from(accountState.activeAccount!.balanceList!.where((element) => element.token == widget.assetPair.tokenA))[0];
                var quoteBalance = List.from(accountState.activeAccount!.balanceList!.where((element) => element.token == widget.assetPair.tokenB))[0];
                isErrorBalance = balanceA > baseBalance.balance || balanceB > quoteBalance.balance;
              } catch (err) {
                isErrorBalance = true;
              }
                                  ...
                                  : () => Navigator.push(
                                        ...
                                        PageRouteBuilder(
                                          pageBuilder: (context, animation1, animation2) => LiquidityConfirmation(
                                              ...
                                              baseAmount: double.parse(_amountBaseController.text.replaceAll(',', '.')),
                                              quoteAmount: double.parse(_amountQuoteController.text.replaceAll(',', '.')),
  ...
  void _setAmount(tokensState) {
    ...
    setState(() {
      amount = double.parse(_amountBaseController.text.replaceAll(',', '.')) / (widget.assetPair.reserveA! / widget.assetPair.totalLiquidity!);
    });
  }

  void _setBalances() {
    setState(() {
      balanceA = (balance / widget.assetPair.totalLiquidityRaw!) * widget.assetPair.reserveA!;
      balanceB = (balance / widget.assetPair.totalLiquidityRaw!) * widget.assetPair.reserveB!;
    });
  }
  ...
  void _setShareOfPool() {
    setState(() {
      if (widget.assetPair.totalLiquidityRaw! != 0) {
        shareOfPool = (convertToSatoshi(double.parse(_amountBaseController.text.replaceAll(',', '.'))) / widget.assetPair.totalLiquidityRaw!);
      }
    });
  }
  ...
  bool isDisableSubmit() {
    int amountFrom = convertToSatoshi(double.parse(_amountBaseController.text.replaceAll(',', '.')));
    int amountTo = convertToSatoshi(double.parse(_amountQuoteController.text.replaceAll(',', '.')));

    return balanceFrom == 0 || balanceTo == 0 || amountFrom > balanceFrom || amountTo > balanceTo;
  }

  setMaxAmount(TextEditingController controller, TextEditingController toController, String asset, double reserve, state, tokensState) {
    WidgetsBinding.instance!.addPostFrameCallback((_) {
      setState(() {
        int balance = state.activeAccount!.balanceList!.firstWhere((el) => el.token! == asset).balance!;
